"""Live test: `evo run` an experiment inside a real Hetzner Cloud server.

Skipped unless `EVO_LIVE_TEST_HETZNER=1` and `HCLOUD_TOKEN` are set.
Requires the optional `hcloud` SDK.
"""
...
def _gate() -> None:
    if os.environ.get("EVO_LIVE_TEST_HETZNER") != "1":
        print("SKIPPED (set EVO_LIVE_TEST_HETZNER=1 to enable)")
        sys.exit(0)
    if not os.environ.get("HCLOUD_TOKEN"):
        print("SKIPPED (set HCLOUD_TOKEN to enable)")
        sys.exit(0)
    try:
        import hcloud  # noqa: F401
    except ImportError:
        print("SKIPPED (hcloud SDK not installed)")
        sys.exit(0)
...
def test_evo_run_against_hetzner() -> None:
    workdir = Path(tempfile.mkdtemp(prefix="evo-hetzner-e2e-"))
    ...
    try:
        ...
        t0 = time.monotonic()
        out = _evo(
            ["new", "--parent", "root", "-m", "hetzner e2e baseline",
             "--remote", "hetzner",
             "--provider-config", provider_config],
            cwd=repo,
            timeout=900,
        )
        print(f"--- evo new exp_0000 (provisions Hetzner server): {time.monotonic() - t0:.1f}s ---")
...
def main() -> None:
    _gate()
    print("=== Hetzner single-experiment end-to-end ===")
    test_evo_run_against_hetzner()
    print("LIVE HETZNER E2E OK")
