...
| `tenki` | Tenki Python SDK (`tenki-sandbox`) | If missing, install `evo-hq-cli[tenki]` (or inject `tenki-sandbox` with `pipx`); then `export TENKI_API_KEY=tk_...` and `export TENKI_PROJECT_ID=...` | `auth_token` (or `api_key`), `project_id`, `workspace_id`, `base_url`, `name_prefix`, `image`, `snapshot_id`, `cpu_cores`, `memory_mb`, `disk_size_gb`, `idle_timeout_minutes`, `root`, `timeout_seconds`, `create_timeout_seconds`, `bootstrap_timeout_seconds`, `health_timeout_seconds` |
